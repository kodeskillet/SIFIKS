...
@section('content')
    ...
    <div style="margin-top:20px;">
        <form class="form-signin" method="POST" action="{{ route('admin.login.submit') }}" autocomplete="off">
            <h1 class="h3 mb-3 font-weight-normal text-center">Admin Log In</h1>
            @csrf
            <div class="form-group row">
                <label for="email" class="col-md-4 col-form-label text-md-right">E-Mail Address</label>
                <div class="col-md-6">
                    <input id="email" type="email" placeholder="E.g. alfuzzy@admin.com" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus>
                    @if ($errors->has('email'))
